...
class PeriodicTable(tk.Frame):
    ...
    def __init__(self, master, selected=[]):
        ...
        for el in _pos.keys():
            ...
            self.element[el].grid(row=_pos[el][0] - 1, column=_pos[el][1] - 1, sticky=tk.W + tk.N + tk.E + tk.S)
            n += 1
    ...
    def selectElements(self, element_list):
    ...
    def setColors(self, event):
        ...
        back = self.color(el, sel=0)  # Default value
    ...
    def show(self):
        element_names = _pos.keys()
        element_names.sort()
        selected = []
        for element in element_names:
            if self.c[element]['relief'] == tk.RAISED:
                selected.append(periodicTable[element])
...
class ElementPropertyFrame(tk.Frame):
    def __init__(self, master, element_list):
        tk.Frame.__init__(self, master)
        element_list.sort()
        ...
        tk.Label(self, text='Atomic \nWeight').grid(column=2, row=0, sticky=tk.W + tk.S,
                                                    padx=10, pady=10)
        for n, element in enumerate(element_list):
            tk.Label(self, text=element.name).grid(column=0, row=n + 1, sticky=tk.W,
                                                   padx=10)
            tk.Label(self, text=repr(element.atomicNumber)).grid(column=1, row=n + 1,
                                                                 sticky=tk.W, padx=10)
            tk.Label(self, text=repr(element.atomicWeight)).grid(column=2, row=n + 1,
                                                                 sticky=tk.W, padx=10)
