    ...
    def make_entries(self):
        ...
        for sp in data_keys:
            ...
            l = SpeciesInfo(self.entries, species=s,
                            text=spname, relief=tk.FLAT, justify=tk.RIGHT,
                            fg='darkblue')
...
class ReactionPathFrame(tk.Frame):
    ...
    def __init__(self, top):
        ...
        self.sc = tk.Scale(scframe, variable=self.thresh,
                           orient='horizontal', digits=3, length=300,
                           resolution=0.01)
        ...
        for i, e in enumerate(enames):
            tk.Radiobutton(eframe, text=e, variable=self.el, value=i,
                           command=self.show).grid(column=i + 1, row=0, sticky=tk.W)
        ...
        for i, e in enumerate(['svg', 'png', 'gif', 'jpg']):
            tk.Radiobutton(fmtframe, text=e, variable=self.fmt, value=e,
                           command=self.show).grid(column=i + 1, row=1, sticky=tk.W)
        ...
        tk.Entry(pframe, width=60, textvariable=self.dot).grid(column=0, row=1,
                                                               sticky=tk.W)
